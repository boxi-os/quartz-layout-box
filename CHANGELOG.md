...
- An encoded placeholder whose name resolves to nothing is no longer decoded. `%7B%7Bunknown%7D%7D`
  and `%7B%7B%7D%7D` came out as `{{unknown}}` and `{{}}` — raw braces where a valid URL had stood,
  because the rewriting accepted any name while the replacement only accepts one it knows. Both
  spellings are now equally conservative: a token is turned back only when it is actually going to
  be replaced, which is what the plain spelling already did.
